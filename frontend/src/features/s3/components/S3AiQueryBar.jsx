...
import { runS3Investigation } from '../api/s3Api'
import { AgentMessage } from '@/components/chat/AgentMessage'

const STORAGE_KEY = 's3-ai-conversation'

export function S3AiQueryBar({ bucketName = null, prefix = '' }) {
    ...
    <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card/80 to-card/50 backdrop-blur-sm ring-1 ring-white/5 flex flex-col overflow-hidden min-h-[120px] hover-lift">
      ...
      <form onSubmit={handleSubmit} className={`flex gap-2 px-4 py-3 bg-gradient-to-r from-primary/5 to-transparent ${messages.length > 0 ? 'border-t border-border/50' : ''}`}>
        <Input
          placeholder={
            messages.length === 0
              ? bucketName
                ? `Ask about this bucket — e.g. "Find large files", "Show patterns"`
                : 'Ask about your S3 buckets — e.g. "Which buckets are using the most storage?"'
              : 'Follow up…'
          }
          value={input}
      ...
      {messages.length === 0 && (
        <p className="text-xs text-muted-foreground px-4 pb-3">
          {bucketName
            ? `Analyze ${bucketName}${prefix ? ` (${prefix})` : ''} — ask about patterns, file sizes, naming conventions.`
            : 'Analyze your S3 buckets with natural language. Ask about storage, permissions, or objects.'}
        </p>
      )}
    </div>
  )
