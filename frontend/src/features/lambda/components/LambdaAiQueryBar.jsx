...
import { runLambdaInvestigation } from '../api/lambdaApi'
import { AgentMessage } from '@/components/chat/AgentMessage'

const STORAGE_KEY = 'lambda-ai-conversation'

export function LambdaAiQueryBar({ functionName = null }) {
  ...
  async function handleSubmit(e) {
    ...
    const queryForAgent = functionName
      ? `[Context: the user is currently viewing the Lambda function "${functionName}" — assume questions refer to it unless they name a different function.]\n${userText}`
      : userText
    setInput('')
    ...
    try {
      const data = await runLambdaInvestigation(queryForAgent, history)
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card/80 to-card/50 backdrop-blur-sm ring-1 ring-white/5 flex flex-col overflow-hidden min-h-[120px] hover-lift">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
        ...
        <span className="text-sm font-semibold">Lambda AI Analyzer</span>
        {functionName && (
      ...
      <form onSubmit={handleSubmit} className={`flex gap-2 px-4 py-3 bg-gradient-to-r from-primary/5 to-transparent ${messages.length > 0 ? 'border-t border-border/50' : ''}`}>
        <Input
          placeholder={
            messages.length === 0
              ? functionName
                ? `Ask about this function — e.g. "Why is this failing?", "Invoke with a test payload"`
                : 'Ask about your Lambda functions — e.g. "Which functions are erroring?"'
              : 'Follow up…'
          }
          value={input}
      ...
      {messages.length === 0 && (
        <p className="text-xs text-muted-foreground px-4 pb-3">
          {functionName
            ? `Analyze ${functionName} — ask about config, triggers, permissions, or invoke it to debug an error.`
            : 'Analyze your Lambda functions with natural language. Ask about config, triggers, errors, or invoke one to test it.'}
        </p>
      )}
    </div>
  )
