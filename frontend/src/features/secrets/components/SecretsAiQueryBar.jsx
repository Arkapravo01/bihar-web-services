...
import { runSecretsInvestigation } from '../api/secretsApi'
import { AgentMessage } from '@/components/chat/AgentMessage'

const STORAGE_KEY = 'secrets-ai-conversation'

export function SecretsAiQueryBar({ secretName = null }) {
  ...
  async function handleSubmit(e) {
    ...
    const queryForAgent = secretName
      ? `[Context: the user is currently viewing secret "${secretName}" — assume questions refer to it unless they name a different secret.]\n${userText}`
      : userText
    setInput('')
    ...
    try {
      const data = await runSecretsInvestigation(queryForAgent, history)
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card/80 to-card/50 backdrop-blur-sm ring-1 ring-white/5 flex flex-col overflow-hidden min-h-[120px] hover-lift">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
        ...
        <span className="text-sm font-semibold">Secrets AI Advisor</span>
        {secretName && (
      ...
      <form onSubmit={handleSubmit} className={`flex gap-2 px-4 py-3 bg-gradient-to-r from-primary/5 to-transparent ${messages.length > 0 ? 'border-t border-border/50' : ''}`}>
        <Input
          placeholder={
            messages.length === 0
              ? secretName
                ? `Ask about this secret — e.g. "What's in this?", "When was it last rotated?"`
                : 'Ask about your secrets — e.g. "Which secrets aren\'t rotating?"'
              : 'Follow up…'
          }
          value={input}
      ...
      {messages.length === 0 && (
        <p className="text-xs text-muted-foreground px-4 pb-3">
          {secretName
            ? `Analyze ${secretName} — ask about its structure, rotation, or tags. It will only reveal the value if you ask.`
            : 'Analyze your secrets with natural language. Ask about rotation, naming, or structure.'}
        </p>
      )}
    </div>
  )
