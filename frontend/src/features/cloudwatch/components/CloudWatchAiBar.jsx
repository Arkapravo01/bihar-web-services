...
const STORAGE_KEY = 'cloudwatch-ai-conversation'

export function CloudWatchAiBar({ logGroupName = null, logStreamName = null, events = [] }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const data = await runInvestigation(enhancedQuery, history)
    ...
    <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card/80 to-card/50 backdrop-blur-sm ring-1 ring-white/5 flex flex-col overflow-hidden min-h-[120px] hover-lift">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
        ...
        {logStreamName && (
          <span className="text-xs text-muted-foreground font-mono bg-primary/10 rounded-lg px-2.5 py-0.5 ml-auto mr-auto ring-1 ring-primary/20">
            {logStreamName.split('/').pop().slice(0, 24)}
          </span>
        )}
        <span className="text-[10px] text-primary font-bold rounded-full border border-primary/30 bg-primary/5 px-2.5 py-0.5 ml-1 uppercase tracking-widest">Phase 2</span>
        {messages.length > 0 && (
      ...
      <form onSubmit={handleSubmit} className={`flex gap-2 px-4 py-3 bg-gradient-to-r from-primary/5 to-transparent ${messages.length > 0 ? 'border-t border-border/50' : ''}`}>
        <Input
          placeholder={
            messages.length === 0
              ? logStreamName
                ? `Ask about this stream — e.g. "What errors are here?", "Any patterns?"`
                : 'Investigate your logs — e.g. "Are there errors in the email service?"'
              : 'Follow up…'
          }
          value={input}
      ...
      {messages.length === 0 && (
        <p className="text-xs text-muted-foreground px-4 pb-3">
          {logStreamName
            ? 'Analyze errors, patterns, or specific events in this stream.'
