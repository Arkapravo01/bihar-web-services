...
import { runRdsInvestigation } from '../api/rdsApi'
import { AgentMessage } from '@/components/chat/AgentMessage'

const STORAGE_KEY = 'rds-ai-conversation'

export function RdsAiQueryBar({ instanceId = null }) {
  ...
  async function handleSubmit(e) {
    ...
    const queryForAgent = instanceId
      ? `[Context: the user is currently viewing DB instance "${instanceId}" — assume questions refer to it unless they name a different instance.]\n${userText}`
      : userText
    setInput('')
    ...
    try {
      const data = await runRdsInvestigation(queryForAgent, history)
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card/80 to-card/50 backdrop-blur-sm ring-1 ring-white/5 flex flex-col overflow-hidden min-h-[120px] hover-lift">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
        <span className="text-base float-icon">🗄️</span>
        <span className="text-sm font-semibold">RDS AI Advisor</span>
        {instanceId && (
      ...
      <form onSubmit={handleSubmit} className={`flex gap-2 px-4 py-3 bg-gradient-to-r from-primary/5 to-transparent ${messages.length > 0 ? 'border-t border-border/50' : ''}`}>
        <Input
          placeholder={
            messages.length === 0
              ? instanceId
                ? `Ask about this instance — e.g. "Is it multi-AZ?", "Take a snapshot"`
                : 'Ask about your databases — e.g. "Which instances are stopped?"'
              : 'Follow up…'
          }
          value={input}
      ...
      {messages.length === 0 && (
        <p className="text-xs text-muted-foreground px-4 pb-3">
          {instanceId
            ? `Analyze ${instanceId} — ask about config, storage, or snapshots. It will confirm before stopping or deleting anything.`
            : 'Analyze your RDS instances with natural language. Ask about status, storage, or backups.'}
        </p>
      )}
    </div>
  )
